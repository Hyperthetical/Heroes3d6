...
Opposed \textlf{might} check with target. If you win, they cannot move, otherwise you suffer a \textlf{moment of weakness} on \textlf{critical failure}. The victim must make an opposed \textlf{Might} check (and spend 1 action point) to escape. It costs 1 action point each round after the first to maintain a grapple. Anyone attacking participants has \textlf{edge-} on \textlf{aim}.

\subsubsection{Trip (1/2)}
Attempt to entangle your foe's legs with your weapon's haft, or pointy bit built for tripping. This modifies a close-combat attack: replace a \textlf{damage check} with an \textlf{opposed roll} between yourself and the target using \textlf{cunning}. Should you win, the target is \textlf{knocked down}. 
...
A character can swap weapons for 1 action point per weapon being swapped.
...
Attacking with two weapons costs an extra action point.

\subsubsection{Penetrating hits}
These are scored if a fighter fails a \textlf{Deflect} attempt by $\dicecritlvl$ or more. A \textlf{Penetrating Hit} grants \textlf{edge+} on the subsequent \textlf{damage checks}.
