...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Success means target suffers \textlf{edge} penalty to rolls for 1 round
	\end{itemize}
\end{itemize}
...
If the target of an attack loses the \textlf{opposed roll} of \textlf{aim} vs \textlf{deflect} by $\dicecritlvl$ or more, then the attacker has achieved a \textlfirst{penetrating hit}. This allows associated \textlf{damage} checks to gain one \textlf{edge} bonus per \textlf{critical success} threshold. 
...
A character armed with a ranged weapon can often attack enemies that are in different combat areas to itself. How far it is allowed to do so is defined by the weapon's range score. A range of 1 would allow shots to be fired at a target in any adjacent area to that of the firer. To determine the required range to a target, simply draw a straight line between it and the shooter then count the number of area boundaries it passes over. These weapons are divided into two types: shooting and throwing. Throwing a weapon/object without the \textlf{throw} property incurs an \textlf{edge} penalty on \textlf{aim}. Firing a shooting weapon at a target within range 0 incurs a similar penalty. 

\subsubsection{All-out attack}
The character channels all their energy into aggression. This attack action can be made with any weapon that has no \textlf{reload} rule, it costs 2 action points but grants \textlf{burst} + 1 to the attack (for one of the character's weapons if wielding two).
...
If a character suffers a successful \textlf{damage} check from a weapon with \textlf{crushing lethality} they are \textlf{badly-wounded}. They suffer an \textlf{edge} penalty on all rolls until remedied.
...
A fighter can try and predict their opponent's next move. This costs 1 action point and requires an \textlf{opposed roll} between yourself and the target using \textlf{cunning}. If you win, then you can change your declared actions in response to the target. \textlf{critical Failure} means you suffer a \textlf{moment of weakness}.
...
\textlf{Grappled} foes cannot make any movement actions until they break free of the \textlf{grapple}, this costs 1 action point and requires that they win a \textlf{might}-based \textlf{opposed roll} with the grappler. If they are forced to move by some external effect then the grapple breaks automatically. All participants have an \textlf{edge} penalty on \textlf{aim}, unless using \textlf{small} or natural weapons.
...
\subsection{Run}
A character can run, at a cost of 2 action points, they then add 1 to their movement range and gain an \textlf{edge} bonus on \textlf{deflect} for 1 round. 
...
\label{sec:dark}
The lighting of an environment greatly influences the difficulty of fighting within it. There are three types of lighting: \textlfirst{full}, \textlfirst{low}, \textlfirst{dark}. \textlf{full} lighting has no effect, \textlf{low} incurs an \textlf{edge} penalty on \textlf{aim} and \textlf{awareness}, and \textlf{dark} means the character cannot see at all outside of 5 m (radius 0) and also incurs \textlf{low} light penalties.
...
Such attacks gain an \textlf{edge} bonus to \textlf{damage} checks when made with \textlf{small} weapons, such as daggers, or with specialist tools, such as garrotte wire. Multiple \textlf{perks} can improve \textlf{sneak attacks}. \textlf{Small} weapon bonuses are lost on \textlf{Huge creatures} or larger targets.
...
An enraged character seeks to enter close combat with the nearest enemy at all times and cannot take prisoners, they go only for the kill. If wielding a firearm-type weapon, they may fire it at range (while moving as required). Enraged characters have an \textlf{edge} penalty on \textlf{deflect} and but an \textlf{edge} bonus on \textlf{damage} checks. Additionally, reduce the \textlf{lethality} of any damage suffered by one level, to a minimum of normal. This state ends when there are no enemies left to defeat in the vicinity. 

\subsection{Moment of Weakness}
\label{sec:aoo}
These are moments when an opponent opens their guard or shows vulnerability, allowing an attacker to strike unhindered. A creature subject to a \textlf{moment of weakness} suffers an immediate \textlf{damage} check from one enemy within reach (radius 0 for human-sized creatures). Sometimes, the enemy who may exploit the free hit is specified by the effect, otherwise the eligible enemy with the highest \textlf{cunning} gets the hit.
Each creature or player can make only one of this type of attack per round. 

\subsection{Resisting effects}
Some attacks/powers have special effects. These should always be potentially mitigated via \textlfirst{resist}. This involves an \textlf{opposed roll} between the \textlf{power} of the effect ($\dicenoprof$ or the user's \textlf{Might} if \textlf{proficient}) and the most appropriate choice from the victim's attributes. For instance, resisting poison might use \textlf{Resolve}, whereas avoiding being hypnotised would require \textlf{wit}.    
...
%
%A mounted charge grants the rider an \textlf{edge} bonus to \textlf{damage} checks this round.
%
...
A powerful blow can sometimes open up a fighter to being easily finished off. All attacks directed at the victim benefit from an \textlf{edge} bonus to \textlf{damage} checks for the effect's duration (default is 1 round).
...
A creature that is \textlfirst{knocked down} loses its footing and falls over. Such a creature has an \textlf{edge} penalty on \textlf{deflect} until it can stand up. Standing up can be done only in your own turn. It costs no action points if the character wins a \textlf{might}-based \textlf{opposed roll} with all adjacent opponents, otherwise it costs 1 action point. Characters in \textlf{heavy armour} suffer an \textlf{edge} penalty on the opposed roll.
...
An \textlfirst{immobilised} creature cannot move, either due to some magical malaise or due to having suffered damage to its legs. In addition, they suffer an \textlf{edge} penalty on \textlf{deflect}.
...
\subsubsection{Fear}
If a character is afflicted by this effect they must make a \textlf{resolve} check against a \textlf{difficulty}, chosen by the GM, based on how scary the situation is. Should the character fail, they suffer an \textlf{edge} penalty on \textlf{aim}, each subsequent turn the character may try to pass the check again. 
...
A weapon with \textlfirst{massive damage} does exactly as their name suggests. Thus, they have an \textlf{edge} bonus on their \textlf{damage} checks, making mayhem all the more likely. 
...
All characters dual-wielding have an \textlf{edge} penalty to \textlf{aim} without the \textlf{Two-Weapon Fighting proficiency}. This does not apply if the weapons aren't used simultaneously. Finally, a \textlf{proficient} character may swap two weapons at once for a single action point.
...
This allows the character to fight with two weapons simultaneously without an \textlf{edge} penalty on \textlf{aim} checks. Additionally, the character may swap two weapons at once for a single action point.
...
This costs two action points and allows the character to make a ranged attack with a single \textlf{damage} check only. This benefits from an \textlf{edge} bonus on \textlf{Aim} and the \textlf{damage} check.


\subsection{Berserker [3]}
You mad, bro? The character can quickly be overtaken by the pulse of battle, they are forced to \textlf{enrage} if they reduce an enemy to 0 \textlf{endurance} or inflict/suffer a \textlf{wound} effect.
...
(Requires \textlf{berserker}) The character's rage is turns them into a highly focussed killer. This grants the \textlf{enraged} character a \textlf{lethality} bonus against an enemy chosen when they \textlf{enrage}, while this enemy survives they suffer an \textlf{edge} penalty to \textlf{aim} versus any other target.
...
At a cost of 1 action point, the character's \textlf{sneak attacks} \textlf{damage} checks gain a bonus level of \textlf{critical success} this turn.
...
(Requires \textlf{Lightning Fast}) The character has an \textlf{edge} bonus on \textlf{aim} and \textlf{damage} checks during a turn when they have triggered \textlf{Lightning fast}.
...
Enemy not dying? Just hit it harder! The character can declare a close-combat attack as a ``power attack''. This incurs an \textlf{edge} penalty on \textlf{aim}, but grants \textlf{massive damage} to associated \textlf{damage} checks.
...
A close quarters manoeuvre where a quick punch/kick is used to make room for firing a ranged weapon. This special attack costs 2 action points. Make an unarmed attack, if it hits you may perform \textlf{damage} checks for both the unarmed hit and for a shooting-type, ranged weapon you are wielding. Note that it must be fireable with one hand (so no bows, but cross-bows or firearms are fine). 
...
A surge of adrenaline can grant a fighter great power in times of danger. If the character loses \textlf{endurance} to a \textlf{damage} check, they gain an \textlf{edge} bonus to their next \textlf{damage} check.
...
Is a good defence. When the character scores \textlf{critical success} on \textlf{deflect}, their opponent suffers a \textlf{moment of weakness}. This can be used once per round.
...
Now the bulls run from you. The character has learned to enter combat with unstoppable momentum. If the character moves before making a \textlf{shove} action (See Section~\ref{sec:adv-spc}), they have \textlf{edge} on the \textlf{opposed roll} . 
...
(Requires \textlf{Skill proficiency: leadership}) Whenever the character succeeds on a \textlf{leadership} check in combat, an ally within range 0 of the character gains 1 point of missing \textlf{endurance}.
...
Better than other plans. When making \textlf{sneak attacks}, the character gains a\textlf{power} bonus equal to their \textlf{cunning}.
...
(Requires \textlf{Duelist}) While fighting with a single weapon (as detailed in \textlf{Duelist}), the character gains an \textlf{edge} bonus to their next \textlf{deflect} after scoring a \textlf{penetrating hit}. 

\subsubsection{Upgrade: Swash Their Buckle [2]}
(Requires \textlf{Duelist}) The character may gain an \textlf{edge} bonus on \textlf{aim} against any target whose attacks they \textlf{deflect} with \textlf{critical success}. 
...
It's hard to pin you down. This grants the character + 1 \textlf{deflect} provided they are wearing \textlf{light armour} or no armour.
...
The character is skilled at moving and fighting in the chosen environment (X). This grants them an \textlf{edge} bonus to stealth, as well as \textlf{survival} checks while in this setting. The character is also not affected by \textlf{rough} or \textlf{dangerous terrain} associated with this environment.
...
When mounted/driving and moving, the character has an \textlf{edge} bonus to \textlf{deflect}.


\subsection{Nimble [3]}
\textlfirst{Nimble} characters are quick and agile. Their movement actions have +1 range while wearing \textlf{light}, or no, armour.
...
Carrying too much around can make moving or fighting rather difficult. To reflect this, characters carrying a large back-pack, or wearing heavy armour, suffer an \textlf{edge} penalty to deflect.
