...
Fabrication-type skills require material components applicable to the item you wish to create; they also require time based on how complex the effort is. The value of the materials should be around $70\%$ of the listed cost of the item or of what it would cost from a reasonable merchant (Game Master discretion). For more difficult items the profit margin can be made larger.
...
If the character is \textlf{proficient} with their current weapon, the \textlf{Power} is the character's \textlf{might} plus the bonus conferred by the weapon or ability being used. Otherwise, just use $\dicenoprof +$ the bonus conferred by the weapon or ability.
...
All thrown weapons can be used effectively at point blank range (range 0). Throwing two \textlf{small} projectiles (like daggers) from the same hand incurs an \textlf{edge} penalty to the thrower's \textlf{aim}.
...
\subsubsection{Upgrade: Savage Reprisal (4)}
...
\subsubsection{Upgrade: Or Swash Their Buckle (2)}
