...
These enemies are followers, they need to be lead. Without a leader to direct them, they are likely to be easily demoralised by powerful opponents or to simply attack the enemy they feel is the most threatening. Such behaviour is over-ridden in the presence of a leader. Mundane enemies suffer the \textlf{panic} effect (see section~\ref{sec:special}) if they run out of \textlf{endurance} or suffer a wound effect. Enemies who believe in their cause, or are mindless, have immunity to this. When being lead, a mundane creature may use its leader's leadership score to make \textlf{panic} checks.
...
\textlf{Passive perk}. Don't tell me what to do. This allows the character \textlf{edge+} on checks against effects that would inflict \textlf{staggered}, \textlf{stunned}, \textlf{immobilised}, or \textlf{knocked-down}.
...
\textlf{Passive perk}. The character can spend a reaction point to gain the effect of \textlf{defensive fighting} when they are targeted by an attack. 
...
\textlf{Passive perk}. The character is so sharp they can evade even unseen attacks at the very last minute. This allows the character to attempt \textlf{deflect} against \textlf{sneak attacks}, however, they suffer \textlf{edge-} on this.


\subsection{True grit [4]}
\textlf{Passive perk}. Pain is just information. When the character is reduced to 0 \textlf{endurance} they can spend a reaction point to regain 1 \textlf{endurance}.


\subsection{Your mother was a hamster [3]}
\textlf{Passive perk}. The character has great knowledge of insults and is a master of taunting jibes. The character has \textlf{edge+} on \textlf{resolve} checks intended to taunt or provoke a target into attacking them.
...
\textlf{Passive perk}. A surge of adrenaline can grant a fighter great power in times of danger. If the character loses \textlf{endurance} to a \textlf{damage check}, they can spend a reaction point to gain \textlf{edge+} to their next \textlf{damage check}.
...
\textlf{Passive perk}. You mad, bro? The character can quickly be overtaken by the pulse of battle, they can spend a reaction point to \textlf{enrage} if they take or deal any damage.
...
\textlf{Active perk}. The character aims their strikes to maim the target. This is an attack action that costs 2 action points. Should the attack cause damage, the target is subject to the \textlf{stunned} and \textlf{staggered} effects. This effect applies only to creatures a maximum of 1 size category larger than the character.


\subsection{Executioner [3]}
\textlf{Passive perk}. The character has learned to deal death swiftly when the chance arises. As such, when striking an opponent who suffered a \textlf{critical hit} this round, the character benefits from a \textlf{lethality} upgrade. This effect applies only to creatures a maximum of 1 size category larger than the character.
...
\textlf{Passive perk}. The character shows no mercy, dispatching the weak with even greater abandon. \textlf{damage checks} against \textlf{bleeding}, \textlf{staggered}, or \textlf{stunned} targets gain a \textlf{lethality} bonus.
...
\textlf{Passive perk}. The character gets \textlf{edge+} to \textlf{aim} versus enemies who have suffered damage this round.
...
\textlf{Active perk}. A close quarters manoeuvre where a quick blow with a fist, or the weapon itself, is used to make room for firing a ranged weapon. This special attack costs 2 action points. Make an unarmed attack, if it hits you may perform \textlf{damage checks} for both the unarmed hit and for a shooting-type, ranged weapon you are wielding. 


\subsection{Whirlwind of steel [3]}
\textlf{Active perk}. Danger, sharp objects. The character lashes out around them, striking all too slow to escape. This costs 2 action points and allows the character to make an attack action with their close-combat weapons that has \textlf{cleave} + 1.
...
\textlf{Passive perk} (Requires \textlf{Weapon proficiency: unarmed}). The best weapon is you. This extends the effects of \textlf{Snake Fist}, \textlf{Shatter Strike}, and \textlf{Dragon Kick} to a single chosen close-combat weapon type.
...
\textlf{Passive perk}. This allows the character to treat any weapon or improvised projectile as though it had the \textlf{throw} rule. 


\subsection{Take-down [2]}
\textlf{Passive perk}. If you make a \textlf{shove} action against a target you have \textlf{grappled} they are slammed to the ground. This causes a successful \textlf{shove} to deal damage with normal \textlf{lethality} in addition to its usual effects. 


\subsection{Turn the tables [4]}
\textlf{Passive perk}. If the character is targeted by a \textlf{grapple} or \textlf{shove} attempt that fails they can spend a reaction point to successfully use this effect on the grappler/shover. 
...
\textlf{Passive perk}. Is a good defence. When the character scores \textlf{critical success} on \textlf{deflect}, they may spend a reaction point so that the attacker suffers a \textlf{moment of weakness}. 
...
\textlf{Active perk}. When the character hits with a close-combat attack they may spend a reaction point to gain the benefit of both \textlf{trip} and \textlf{disarm} weapon rules. If the weapon already has one of these rules, it gains one bonus level of \textlf{critical success} if the corresponding action succeeds.
...
\subsection{Plan B [3]}
\textlf{Passive perk} (Requires \textlf{Duelist}). Whenever one of the character's attacks is \textlf{deflected}, while in \textlf{Duelist} mode, they may spend a reaction point to draw and throw/fire a \textlf{small} weapon (this doesn't provoke a \textlf{moment of weakness} or suffer penalties for close range).
...
\textlf{Passive perk}. This allows the character to fight with two weapons simultaneously without \textlf{edge-} on \textlf{aim} checks. 
...
\textlf{Passive perk}. The character can spend a reaction point to use a \textlf{leadership} action at any time during combat.
...
\section{Mobility}
These \textlf{perks} are oriented towards movement in combat. They must occupy an equipment slot for active/passive \textlf{perks} to be usable.
\label{sec:perks-movement}

\subsection{Acrobatic attack [4]}
\textlf{Active perk} (Requires \textlf{Acrobatic movement}). This costs 1 action point and allows the character to make both an \textlf{acrobatic movement} and attack action (against a target within range during the movement).% This action cannot be made in either \textlf{medium} or \textlf{heavy armour}.
...
\textlf{Passive perk} (Requires \textlf{Skill proficiency: perform}). It pays to be prepared. The character can produce a single extra dagger (or other \textlf{small} throwing weapon) in an emergency. This can be used at most once per day.


\subsection{Arrow to the knee [4]}
\textlf{Passive perk}. A careful shot to the leg can topple the most stable foes. The character can make \textlf{trip} actions with ranged weapons (these count as having the \textlf{trip} rule).


\subsection{Blind side [3]}
\textlf{Active perk}. A deft feint allows the character enough room to fire their ranged weapons against close-combat assailants. When using a ranged weapon against a target within range 0 you may feint as though you were using a close-combat weapon. 


\subsection{Chink in the armour [3]}
\textlf{Passive perk}. Close-combat attacks against enemies who are \textlf{knocked down} or \textlf{grappled} gain \textlf{penetration} + 2 at a cost of 1 reaction point.
...
A\textlf{Passive perk}. At a cost of 1 reaction point an attack against a \textlf{knocked down} target becomes a \textlf{sneak attack}.
...
\textlf{Active perk}. The character reacts so quickly that, during action declaration in combat, they may seize the initiative for their side at a cost of 1 reaction point and 1 \textlf{heroism}.
...
\textlf{Passive perk}. If the character succeeds on a \textlf{feint} action, then associated \textlf{damage checks} gain any benefits that apply to \textlf{sneak attacks}.
...
\textlf{Passive perk}. The character can draw/change weapon at a cost of 1 reaction point.
...
\textlf{Passive perk}. A precisely placed shot causes the target to drop their weapon. The character can make \textlf{disarm} actions with ranged weapons (these count as having the \textlf{disarm} rule).
