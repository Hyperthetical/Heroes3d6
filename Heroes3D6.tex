...
Reflects the character's resistance to physical harm. This comes exclusively from a character's armour unless otherwise specified. Unarmoured characters have a base \textlf{Toughness} of 8, this score is increased by the  armour they wear. 
...
If a character suffers a successful damage roll from a weapon with \textlf{crushing lethality} he is \textlf{badly-wounded}. He suffers an \textlf{edge} penalty on all rolls until remedied. Once remedied, healing checks made on the character have $+1$ \textlf{difficulty} for 10 days. 
...
A \textlf{mortally-wounded} character cannot make actions and will die in three days unless he receives some medical attention. This status can only be removed by healing.
...
Declare a \textlf{disarm} attack before the \textlf{deflect} check is made. Should the attack land, it causes no damage roll but renders the victim \textlf{vulnerable} (see Section~\ref{sec:special}) to the next damage roll if it fails an opposed \textlf{Wit} check. Weapons with the \textlf{disarm} rule can replace \textlf{moment of weakness} damage rolls with a \textlf{disarm} action instead.
...
The character has trained to use heavy armour effectively. This negates the \textlf{deflect} penalty when wearing such armour.

\subsection{Hero/Villain (0)}
(Requires \textlf{Reputation} level 5) The character has become famous, his praise is much sung, or his vile deeds whispered of, in local taverns. The character may increase one \textlf{natural attribute} by 1 point.
...
(X must be a non-combat skill) The character has achieved mastery of his art. Any check for the mastered skill X an \textlf{edge} bonus.
...
The character has learned to deal death swiftly when the chance arises. As such, when striking an opponet with zero \textlf{endurance}, the character benefits from a \textlf{lethality} upgrade. This effect applies only to creatures a maximum of 1 size category larger than the character.
...
The character may also make \textlf{sneak attacks} against enemies who have zero \textlf{endurance}, even if they don't meet any other pre-requisites for \textlf{sneak attacks} (see Section~\ref{sec:sneak-attk}).
...
(Requires Unarmed Training) While wearing light or no armour the character may add his \textlf{Wit} score to his \textlf{toughness}.
