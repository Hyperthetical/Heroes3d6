...
Some effects cause a creature to lose action points. Note that this is \textbf{always} to a minimum of 1 and applies on/from the victim's \textbf{next} turn, as during \textlf{action resolution} they have spent their points already.
...
\subsection{Movement}
Most creatures may transfer between combat areas that share a boundary unless otherwise noted or modified by terrain. Movement actions cost 1 action point if the character wishes to change combat area. Normal movement in this manner is thus said to have a range of 1. Leaving a combat area that contains enemies results in a \textlf{Moment of weakness} (see Section~\ref{sec:aoo}).
...
The character channels all their energy into aggression. This attack action costs 2 action points but grants \textlf{burst} + 1 to the attack (this grants an extra \textlf{damage check}, see Section~\ref{sec:special}). If the character is wielding multiple weapons then only one of them gets the bonus.
...
\subsection{Taunt}
With cutting words, or obscene gestures, you bait a foe into attacking you. This costs 1 action point and the target must be able to understand the provocation in some way. Make an opposed check with your \textlf{resolve} and the target's \textlf{wit}. If you win, the target now attempts to attack you instead of other targets. 
...
\label{sec:rough-t}
This includes: loose rocks, tree roots, rubble, tables and chairs, obstructions of about knee or waist height, and surfaces that are unstable/moving/irregular. Moving through \textlf{rough terrain} costs 1 action point more than normal. The character can still make \textlf{run} actions for 2 action points, but they must pass an \textlf{athletics} check of difficulty between 8 and 12 (depending on how rough the terrain is) or suffer damage with \textlf{lethality} 1 + 1 per level of \textlf{critical failure}.
...
A slowed creature has had their movement impeded. This reduces the victim's movement range by 1 (to a minimum of 0).
...
(X is an integer) An attack with \textlfirst{cleave} is one that can sweep in great arcs, cutting through many hapless foes with each swing. Such an attack may target up to 1 + X enemies at once (they all must be within range and in the same combat area). The attacker rolls once with their \textlf{aim} and each defender rolls with their \textlf{deflect}.
...
A creature is declared large if it is bear-sized or larger (2 m in one dimension and 1 m in other dimensions or 3 m in main dimension). Thus, such creatures can only hide in medium or better cover but gain \textlf{cleave} 1 on all close-combat attacks. A \textlf{large creature} has an innate bonus of +1 \textlf{power} in close-combat. Such a creature has at least 4 \textlf{endurance} points.
...
\textlf{Active perk}. The character can spend a reaction point to gain the effect of \textlf{defensive stance} (\textlf{edge+} on \textlf{deflect}) when they are targeted by an attack. 


\subsection{Set to defend [3]}
\textlf{Passive perk}. When the character uses \textlf{defensive stance}, they have \textlf{edge+} on \textlf{deflect} for the next 2 attacks, instead of just 1.
...
\textlf{Active perk}. Pain is just information. The character can spend a reaction point when an attack hits them to confer \textlf{edge-} on any associated \textlf{damage checks}.


\subsection{Grin and bear it [3]}
\textlf{Active perk}. When the character is reduced to 0 \textlf{endurance} they can spend a reaction point to regain 1 \textlf{endurance}.
...
\textlf{Active perk}. A surge of adrenaline can grant a fighter great power in times of danger. If the character loses \textlf{endurance} to a \textlf{damage check}, they can spend a reaction point to gain \textlf{edge+} on their next \textlf{damage check}.

\subsubsection{Upgrade: Savage reprisal [3]}
(Requires \textlf{Adrenaline Rush}). Whenever the character triggers \textlf{Adrenaline Rush} they may make a single, free, close-combat \textlf{damage check} against their assailant (provided it is within reach).


\subsection{Aimed shot [5]}
\textlf{Active perk}. This costs two action points and allows the character to make a ranged attack with a single \textlf{damage check} only (regardless of any other bonuses). This benefits from \textlf{edge+} on \textlf{Aim} and the \textlf{damage check}.
...
\textlf{Active perk}. This attack knocks the foe off balance. This is a normal \textlf{attack} action that costs 2 action points. If the attack causes damage then the target suffers \textlf{edge-} on \textlf{deflect} in the next round.


\subsection{Hamstring [3]}
\textlf{Active perk}. Striking at a foe's legs can all but cripple them. This is a normal \textlf{attack} action that costs 2 action points, victim's damaged by it are \textlf{slowed} for the next 2 rounds.
...
\textlf{Passive perk}. The character gets \textlf{edge+} to \textlf{aim} versus enemies who are subject to any \textlf{condition} (see section~\ref{sec:special}).
...
\textlf{Active perk}. The character lashes out around them, striking all too slow to escape. This costs 2 action points and allows the character to make a close-combat \textlf{attack} action that has \textlf{cleave} + 1.
...
\textlf{Passive perk} (Requires \textlf{Weapon proficiency: unarmed}). The character is swift and dexterous in unarmed combat. Unarmed attacks count as having the \textlf{tripping} and \textlf{disarming} rules.

\subsubsection{Upgrade: Serpent strike [3]}
(Requires \textlf{snake fist}). The character has learned to unleash a volley of strikes with hands and feet. Unarmed attacks make an extra \textlf{damage check}. This does not incur \textlf{dual-wielding} penalties.
...
\textlf{Passive perk} (Requires \textlf{Weapon proficiency: unarmed}). The character gains \textlf{edge+} on \textlf{deflect} while unarmed and wearing no armour.
...
\textlf{Active perk}. When the character hits with a close-combat attack they may spend a reaction point to add an opposed \textlf{trip} or \textlf{disarm} roll without sacrificing a \textlf{damage check}.

\subsubsection{Upgrade: Flourish [3]}
...
\textlf{Active perk}. The character may spend a reaction point when they score \textlf{critical success} to \textlf{deflect} an attack, gaining $+1$ \textlf{aim} per level of \textlf{critical success} versus the attacker for 1 round.
...
%The \textlf{lethality} of \textlf{counter attack} is equal to the number of \textlf{critical success} thresholds on \textlf{deflect}.
...
(Requires \textlf{Duelist}). While fighting with a single weapon (as detailed in \textlf{Duelist}), the character gains a reaction point when they score \textlf{critical success} on a \textlf{deflect} roll. This can only happen once per round of combat.

%the character may spend a reaction point to gain \textlf{edge+} to their next \textlf{deflect} after scoring a \textlf{penetrating hit}. 

\subsubsection{Upgrade: Swash their buckle [2]}
(Requires \textlf{Duelist}). While fighting with a single weapon (as detailed in \textlf{Duelist}), if the character hits with an attack they may spend a reaction point to gain \textlf{penetration} +2 for associated \textlf{damage checks}. 
%\textlf{edge+} on the next \textlf{aim} roll after they \textlf{deflect} with \textlf{critical success}. 
...
\textlf{Active perk}. The character can spend a reaction point after resolving an attack (or magical effect) that hits. This allows them to make a \textlf{move} action that cannot trigger a \textlf{moment of weakness}. 
...
\textlf{Passive perk}. Choose two different skills  (you must be proficient in them), add + 1 to rolls made with those skills. This can only be chosen once per character.
...
\textlf{Passive perk}. A careful shot to the leg can make movement painful and difficult. The character can spend an extra action point to cause an \textlf{attack action} to inflict \textlf{slowed} on a damaged victim.
...
(Requires \textlf{Too hot to handle}). The character counts ranged weapons as having the \textlf{disarming} rule.
