...
This rule book is open-source~\footnote{Copyright (C) 2022  Geoff Beck}: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
...
\begin{itemize}
	\item Make a creature ``snap out of it'suspicious of your intentions'
	\begin{itemize}
...
Such a hit represents a flesh-wound and inflict \textlf{staggered} on the victim (see \ref{sec:special}). The \textlf{wound} effect itself lasts until healed. 
...
A character that has suffered wounds can be healed via use of the \textlf{Healing skill} or by a non-player healer. Without healing, a character heals one \textlf{wounded} effect every 3 days and one \textlf{badly-wounded} every 10 days.
...
The character reacts so quickly that, during action declaration in combat, they may seize the initiative for their side at a cost of 1 reaction point. A character may not use \textlf{Lightning Fast} more than once per encounter (additionally, each side in combat can only benefit from this once per encounter).
...
(Requires \textlf{Quick Draw}) The character's attacks, made after making a surprise \textlf{Quick Draw} of \textlf{small} weapons are \textlf{sneak attacks}. This bonus cannot be claimed outside the first round of (or prior to) a combat situation.
...
Pain is just information. When the character loses \textlf{endurance} they can spend a reaction point to regain 2 \textlf{endurance} (up to their usual maximum). 
