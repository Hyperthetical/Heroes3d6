...
(X is an integer) An attack with \textlfirst{cleave} is one that can sweep in great arcs, cutting through many hapless foes with each swing. Such an attack may target up to 1 + X enemies at once (they all must be within range). The attacker rolls once with their \textlf{aim} and each defender rolls with their \textlf{deflect}.
...
A creature is declared large if it is bear-sized or larger (2 m in one dimension and 1 m in other dimensions or 3 m in main dimension). Thus, such creatures can only hide in medium or better cover but gain \textlf{cleave} 1 on all close-combat attacks. A \textlf{large creature} has an innate bonus of +1 \textlf{power}. Such a creature has at least 4 \textlf{endurance} points.
...
\subsection{Counter attack [6]}
\textlf{Active perk}. The character may spend a reaction point when they score \textlf{critical success} to \textlf{deflect} an attack. This inflicts damage with \textlf{lethality} 1 to the attacker.
