...
\begin{itemize}
	...
	\begin{itemize}
		\item \textlf{difficulty} 8 (tossed ball) to 17 (arrow)
...
\begin{table}
	...
	\begin{tabular}{|l|l|l|}
		...
		\hline
		Lead limb & Victim cannot run and attacks cost 2 \textbf{AP} & \textlf{staggered} \\
...
A slowed creature has had their movement impeded. The victim's movement-type actions cost 1 extra \textbf{AP}. This cannot stack.
...
(X is an integer) \textlfirst{Reach} represents the ability of some close-combat weapons, like pikes, to attack from a longer distance. A character with a \textlf{reach} weapon can make close-combat attack, \textlf{trip}, and \textlf{disarm} actions at range X without paying any additional action cost. %These weapons suffer \textlf{edge-} on \textlf{aim} when targeting foes within range 0. %In addition, entering a combat area (\dist{} radius) containing foes with such weapons incurs a \textlf{Moment of weakness} unless an extra \textbf{AP} is spent when making the \textlf{move}.
...
\textlf{Active perk}. When the character is reduced to 0 \textlf{endurance} they can spend an \textbf{RP} to regain 1 \textlf{endurance} and \textlf{hardened} 1.
...
\textlf{Active perk}. Striking at a foe's legs can all but cripple them. This is a normal \textlf{attack} action that costs 2 \textbf{AP}, victim's damaged by it are \textlf{slowed} for 1 round.
...
True mastery achieved. Instead of its usual effects, success on a \textlf{taunt} action now \textlf{stuns} the target for 1 round and inflicts \textlf{edge-} on all rolls until the target successfully causes damage to you.
