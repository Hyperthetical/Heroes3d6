...
A risky manoeuvre, the fighter replaces a considered attack with a hail of furious strikes. When declaring an attack, the character may elect it to be a \textlf{furious assault}. This grants an extra damage roll with a successful hit. However, the character suffers an \textlf{edge} penalty to \textlf{aim}. This cannot be used if the weapon requires \textlf{reload} actions.
...
\subsubsection{Upgrade: Thews and sinews (3)}
...
\subsection{Martial Skill (4)}
The character counts all close-combat weapons as having the \textlf{trip} and \textlf{disarm} rules. 
