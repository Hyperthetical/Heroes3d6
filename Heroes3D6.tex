...
\item
Now create a \textlfirst{background} following Section~\ref{sec:backgrounds}. This will inform your choice of 2 skills to be \textlf{proficient} in.
\item 
...
\item
Choose two \textlf{attributes} to have the value 1, set all other \textlf{attributes} to 0 (try and make your choices align with the character's personality).  Set \textlf{heroism/villainy} to 1. 
\item 
Spend up to 3 experience points choosing \textlf{perks} or \textlf{proficiencies} (Chapter~\ref{chap:perks}) for your character. You may take one weapon proficiency for free.
\item
...
A healer is one who is skilled is speeding the recovery of their allies through strictly medical/surgical means (potions are the province of the chemist), a healer can employ their healing skill to remove injuries inflicted to an ally. The kind of healing actions are:
...
These skills all deal with interaction with other people.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item The \textlf{difficulty} is given by $11 +$ the \textlf{power} of the condition 
	...
	\begin{itemize}
		...
		\item Make a check with your \textlf{Leadership} vs \textlf{difficulty}
		\item \textlf{difficulty} determined by the directed action
		\item If you succeed, and the ally obeys, the action has \textlf{edge+}
...
\subsection{Aim}
This reflects the ability to successfully penetrate a foe's defences and land telling blows. \textlf{Aim} is given by the character's \textlf{Wit} score plus an bonuses/penalties from their equipped weapon.
...
A character armed with a ranged weapon can often attack enemies that are in different combat areas to itself. How far it is allowed to do so is defined by the weapon's range score. A range of 1 would allow shots to be fired at a target in any adjacent area to that of the firer. To determine the required range to a target, simply draw a straight line between it and the shooter then count the number of area boundaries it passes over. 

These weapons are divided into two types: shooting and throwing. Throwing a weapon/object without the \textlf{throw} property incurs \textlf{edge-} on \textlf{aim}. Firing a shooting weapon at a target within range 0 incurs a similar penalty. 

\subsubsection{All-out attack}
The character channels all their energy into aggression. This attack action can be made with any weapon that has no \textlf{reload} rule, it costs 2 action points but grants \textlf{burst} + 1 to the attack (for one of the character's weapons attacking with two).
...
If the character is using a weapon in each hand, they may attack with both for 1 action point. Only one \textlf{opposed roll} of \textlf{deflect} vs \textlf{aim} is made, but, each weapon gets a separate \textlf{damage check}. If the character wishes to divide their attack between opponents then make separate \textlf{deflect} vs \textlf{aim} checks for each target.

Character's without certain \textlf{perks} (see Section~\ref{sec:dual}) suffer \textlf{edge-} on \textlf{aim} when attacking with two weapons at once.
...
This is a \textlf{difficulty check} with your \textlf{power} against the victim's \textlf{toughness}. If not \textlf{proficient} with their equipped weapon, the character suffers \textlf{edge-} on these checks. If the check succeeds then the victim loses \textlf{endurance}. How many points are lost depends on the \textlfirst{lethality} of the attack (see table~\ref{tab:lethal}). Once a character has no \textlf{endurance} left (minimum is 0), further damage is accumulated as actual injuries which are tracked via \textlf{wound} effects. If a character would be reduced to negative \textlf{endurance}, the lost points beyond zero are converted to wounds (i.e. -1 to \textlf{wounded} or -2 to \textlf{badly-wounded} following table~\ref{tab:lethal}). 

If a \textlf{wounded} character has their \textlf{endurance} restored to values $>0$ there is no effect on their \textlf{wounds}, the latter can only be remedied by the \textlf{healing skill} or time.
...
If a character succeeds on a \textlf{damage check} by $\dicecritlvl$ or more, the attack's \textlf{lethality} is upgraded by 1 level per $\dicecritlvl$ they exceeded it by.

\subsubsection{Wounded}
This represents a moderately serious flesh-wound and inflicts \textlf{staggered} on the victim (see \ref{sec:special}) until healed. 
...
If a character suffers a successful \textlf{damage check} from a weapon with \textlf{crushing lethality}, or accumulates two \textlf{wounded} effects, they become \textlf{badly-wounded}. This results in \textlf{edge-} on all rolls until remedied.

A man-sized character subject to further damage when \textlf{badly-wounded} becomes \textlf{mortally wounded}.

\subsubsection{Mortally wounded}
If a character suffers a successful \textlf{damage check} from a weapon with \textlf{devastating lethality}, or accumulates enough \textlf{wounded} effects, they are \textlf{mortally-wounded}. Such a character cannot make actions and will die in three days unless they receive some medical attention. This status can only be removed by active healing. If a \textlf{mortally wounded} character is healed, change this effect to \textlf{badly wounded}.
...
This costs 1 action point. The character gains \textlf{edge+} to \textlf{deflect} the next attack.
...
This modifies an attack action: replace a \textlf{damage check} with an \textlf{opposed roll} between yourself and the target using \textlf{wit}. Should you win, the target cannot use their current weapon without either spending 1 action point or suffering a \textlf{moment of weakness} to retrieve it. Weapons with the \textlf{disarm} rule cause damage on a \textlf{critical success}.

\subsection{Exploit weakness}
\label{sec:exploit}
This costs 1 reaction point and can be used when an enemy within range 0 experiences a \textlf{moment of weakness} (see Section~\ref{sec:aoo}). This allows the character to make a single close combat \textlf{damage check} against the enemy.

\subsection{Feint}
\label{sec:feint}
A feint costs one action point and requires an \textlf{opposed roll} between yourself and the target using \textlf{cunning}. Success allows you \textlf{edge+} to \textlf{aim} this round, while \textlf{critical failure} results in a \textlf{moment of weakness}. This has no effect with ranged weapons unless otherwise modified.
...
\subsection{Shove}
This costs 1 action point and involves an \textlf{opposed roll} between yourself and the target using \textlf{might}. If the shover is successful, the target is either knocked down or moved into an adjacent combat area (at the shover's discretion). \textlf{critical failure} results in shoving character experiencing a \textlf{Moment of weakness}. \textlf{critical success} results in the victim suffering a \textlf{damage check}.

\subsection{Trip}
Attempt to entangle your foe's legs with your weapon's haft, or pointy bit built for tripping. This modifies an attack action: replace a \textlf{damage check} with an \textlf{opposed roll} between yourself and the target using \textlf{cunning}. Should you win, the target is \textlf{knocked down} (see Section~\ref{sec:special}). Weapons with the \textlf{Trip} rule cause damage on a \textlf{critical success}.
...
A character that has suffered wounds can be healed via use of the \textlf{Healing skill} or by a non-player healer. Without healing, a character recovers from the \textlf{wounded} effect after 3 days. It takes 6 days before the \textlf{badly-wounded} effect is reduced to \textlf{wounded}.
...
\label{sec:cover}
If a combat zone offers cover then any creature within the zone may claim a bonus to \textlf{deflect}. Cover is divided into three categories and examples will be given below.
\begin{table}[ht]
	\centering
	\caption{Cover table}
	\label{tab:cover}
...
Battlements, Large walls, trees, big rocks, barricades, serious furniture (made of stone or steel). These things are built to provide a lot of cover and protection and also grant \textlf{edge+} to rolls to hide in the cover. Objects that provide heavy cover can be broken with a \textlf{might}/\textlf{power} check against \textlf{difficulty} 13-20.
...
\label{sec:dark}
The lighting of an environment greatly influences the difficulty of fighting within it. There are three types of lighting: \textlfirst{full}, \textlfirst{low}, \textlfirst{dark}. \textlf{full} lighting has no effect, \textlf{low} incurs \textlf{edge-} on \textlf{aim} and \textlf{awareness}, and \textlf{dark} means the character cannot see at all outside of 5 m (radius 0) and also incurs \textlf{low} light penalties.
...
A character becomes \textlfirst{enraged} if under extreme emotional strain while fighting, although particularly volatile personalities may become \textlf{enraged} more easily. Otherwise a character with the Berserker \textlf{power} can \textlf{enrage} in the heat of battle.

\subsubsection{When enraged}
An enraged character is immune to \textlf{fear} or \textlf{terror} effects and seeks to enter close combat with the nearest enemy at all times and cannot take prisoners, they go only for the kill. If wielding a firearm-type weapon, they may fire it at range (while moving as required). Enraged characters have \textlf{edge-} on \textlf{deflect} and but \textlf{edge+} on \textlf{damage checks}. Additionally, reduce the \textlf{lethality} of any damage suffered by one level, to a minimum of normal. This state ends when there are no enemies left to defeat in the vicinity. 

\subsection{Moment of weakness}
\label{sec:aoo}
These are moments when an opponent opens their guard or shows vulnerability, allowing an attacker to strike unhindered. A creature subject to a \textlf{moment of weakness} is open to being targeted by the \textlf{Exploit weakness} reaction~\ref{sec:exploit}. This reaction can only be used once for a single \textlf{moment of weakness}.

\subsection{Resisting effects}
Some attacks/powers have special effects. These should always be mitigated via \textlfirst{resist}. This involves an \textlf{opposed roll} between the \textlf{power} of the effect and most appropriate choice from the victim's attributes. For instance, resisting poison might use \textlf{Resolve}, whereas avoiding being hypnotised would require \textlf{wit}.    
...
\subsubsection{Abilities and attributes}
Such fighters are likely to have above average attributes, e.g. two at level 1, with \textlf{aim} of at least $+1$. Elite enemies do not have any heroic attributes but may have multiple combat proficiencies. 
\subsubsection{Tactics}
...
%Certain weapons, such as the lance and horse sword, utilise the power and speed of a mounted charge to the full, inflicting great damage as they smash into the hapless foe. This action costs 2 action points and allows a normal movement and attack action. The \textlf{difficulty} of the required \textlf{ride} check is 11 + 1 for rough and + 2 for \textlf{dangerous terrain} (with a possible dismount on failure). 
%
%A mounted charge grants the rider \textlf{edge+} to \textlf{damage checks} this round.
%
%\subsubsection{Trample}
%You may attempt, while charging, to ride your opponent down and trample over them. This trample attempt requires an \textlf{opposed roll} between your \textlf{aim} and victim's \textlf{deflect}. If the target wins, they evade the trample and you suffer a \textlf{moment of weakness} that the intended victim may exploit. However, if the \textlf{deflect} fails, the target is trampled beneath your mount's hooves/feet/wheels/appendages of motion. This results in the victim being knocked down and taking three \textlf{damage checks} from the mount. You cannot trample a target that is larger than you and your mount.
%
...
The shock of injury is a difficult thing to ignore and can incapacitate even the most hardened fighters. A \textlf{staggered} creature loses 1 reaction point. 

\subsubsection{Knocked down}
A creature that is \textlfirst{knocked down} loses its footing and falls over. Such a creature has \textlf{edge-} on \textlf{deflect} until it can stand up. Standing up can be done only in your own turn. It costs 1 action point. Characters in \textlf{heavy armour} must spend 2 action points.
...
The sudden shock of a well-placed blow can often neutralise retaliation. If a \textlfirst{stunned} creature attempts to make attacks in its next turn these cost 1 more action point than normal. 
...
This state represents severe bleeding. It lasts 2 rounds, each round the victim must make an opposed check: their \textlf{resolve} against your \textlf{might}. Their failure results in the loss of 1 \textlf{endurance} (this does not scale with levels of \textlf{critical failure}). 

\subsubsection{Fear}
If a character is afflicted by this effect they must make a \textlf{resolve} check against a \textlf{difficulty}, chosen by the GM, based on how scary the situation is. Should the character fail, they suffer \textlf{edge-} on \textlf{aim}, each subsequent turn the character may try to pass the check again. 
...
\subsubsection{Reach}
\textlfirst{Reach} represents the ability of close-combat weapons, like pikes, to attack from a longer distance. A character with a \textlf{reach} weapon can make close combat attacks at range 1.

\subsubsection{Rending X}
(X is an integer) A well-placed blow from a \textlfirst{rending} weapon rips deep into the target. \textlf{Penetrating} hits from \textlf{rending} weapons benefit from +X \textlf{Power}. 

%\subsubsection{Massive Damage}
%A weapon with \textlfirst{massive damage} does exactly as their name suggests. Thus, they have \textlf{edge+} on their \textlf{damage checks}, making mayhem all the more likely. 

\subsubsection{Burst X}
(X is an integer) \textlfirst{Burst} represents attacks that explode, spray, splash, or carve into a wide area of flesh. An attack with \textlf{burst} X causes 1 + X \textlf{damage checks} on a successful hit, rather than just one. 
...
\textlfirst{Mindless} creatures have no thoughts, emotions, fear or creativity. Because of this, \textlf{mindless} creatures are immune to many occult powers, as well as the \textlf{fear}, \textlf{terror}, and \textlf{panic} effects. These creatures cannot use any advanced combat rules (no \textlf{flanking}, \textlf{sneak attacks} etc.) apart from \textlf{grapple} and \textlf{shove}. Mindless creatures are considered destroyed when they run out of \textlf{endurance}.
...
\subsection{Throw}
Weapons with the \textlf{Throw} rule are finely balanced for use as projectiles. Thus, weapons without this rule confer \textlf{edge-} to \textlf{Aim} when thrown. 

\subsubsection{Dual-wielding}
\label{sec:dual}
A character can wield a weapon in each hand if they so desire. While employing such an armament the fighter makes only one \textlf{aim} roll, but a separate \textlf{damage check} with each weapon if their attack hits. If either weapon has a \textlf{reload} rule, the reload time is increased by 1 action point. 

All characters dual-wielding have \textlf{edge-} to \textlf{aim} without the \textlf{Two-weapon mastery perk}. This does not apply if the weapons aren't used simultaneously. Finally, a character with \textlf{two-weapon fighting proficiency} may swap two weapons at once for a single action point.
...
\subsection{Master: X [6]}
(Requires \textlf{Skill proficiency: X}) The character has achieved mastery of their art. Any check for the mastered skill X benefits from \textlf{edge+}.
...
This costs two action points and allows the character to make a ranged attack with a single \textlf{damage check} only. This benefits from \textlf{edge+} on \textlf{Aim} and the \textlf{damage check}.


\subsection{Berserker [4]}
You mad, bro? The character can quickly be overtaken by the pulse of battle, they can spend a reaction point to \textlf{enrage} if they take or deal any damage.

\subsubsection{Upgrade: Berserkergang [2]}
(Requires \textlf{berserker}) The character unleashes their inner rage. When \textlf{enraged} the character can spend a reaction point to grant \textlf{burst} + 1 to an attack that hits.  

\subsubsection{Upgrade: Seething rage [3]}
(Requires \textlf{berserker}) The character's rage is turns them into a highly focussed killer. This grants the \textlf{enraged} character a \textlf{lethality} bonus against an enemy chosen when they \textlf{enrage}, while this enemy survives they suffer \textlf{edge-} to \textlf{aim} versus any other target.
...
A deft feint allows the character enough room to fire their ranged weapons against close-combat assailants. When using a ranged weapon against a target within range 0 you may feint as though you were using a close-combat weapon. 


\subsection{Butcher's blow [3]}
The character aims their strikes to maim the target. This is an attack action that costs 2 action points. Should the attack cause damage, the target is subject to the \textlf{stunned} and \textlf{staggered} effects. This effect applies only to creatures a maximum of 1 size category larger than the character.


\subsection{Combat improvisation [4]}
When the character hits with a close-combat attack they may spend a reaction point to gain the benefit of both \textlf{trip} and \textlf{disarm} weapon rules. If the weapon already has one of these rules, it gains one bonus level of \textlf{critical success} if the corresponding action succeeds.


\subsection{Counter attack [5]}
The character may spend a reaction point when they successfully \textlf{deflect} an attack. This allows them to make their own in retaliation (this gains \textlf{edge+} on \textlf{aim} with \textlf{critical success} on \textlf{deflect}). 
...
The character may spend a reaction point to reduce the \textlf{reload} action point costs of ranged weapons by 1. 
...
%At a cost of 1 action point, the character's \textlf{sneak attack} \textlf{damage checks} gain a bonus level of \textlf{critical success} this turn.
...
This attack knocks the foe off balance. The character may declare this special attack for 2 action points. If the attack causes damage then the target suffers \textlf{edge-} on \textlf{deflect} in the next round.


\subsection{Hit and run [3]}
The character can spend a reaction point after resolving a close-combat attack that hits. This allows them to make a normal move action that cannot trigger a \textlf{moment of weakness}. 


\subsection{Interdiction [3]}
The character can spend a reaction point when an adjacent ally is hit with an attack, causing the \textlf{damage checks} to be resolved against themselves instead.
...
At a cost of 1 reaction point an attack against a \textlf{knocked down} target becomes a \textlf{sneak attack}.


\subsection{Lightning fast [4]}
The character reacts so quickly that, during action declaration in combat, they may seize the initiative for their side at a cost of 1 reaction point and 1 \textlf{heroism}.
...
The character zeros-in on their prey. The character may select a target against which they gain \textlf{edge+} on \textlf{aim} and \textlf{deflect}. However, they suffer a similar penalty against all other targets. A new target may only be nominated once the present target is dead, defeated, or hostilities cease.
...
\subsection{Plan B [3]}
(Requires \textlf{Duelist}) Whenever one of the character's attacks is \textlf{deflected}, while in \textlf{Duelist} mode, they may spend a reaction point to draw and throw/fire a \textlf{small} weapon (this doesn't provoke a \textlf{moment of weakness} or suffer penalties for close range).


\subsection{Power attack [4]}
Enemy not dying? Just hit it harder! The character can declare a close-combat attack as a ``power attack''. This incurs \textlf{edge-} on \textlf{aim}, but grants \textlf{edge+} to associated \textlf{damage checks}.


\subsection{Reactive defence [4]}
The character can spend a reaction point to gain the effect of \textlf{defensive fighting} when they are targeted by an attack. 


\subsection{Reckless attack [4]}
The character can declare a close-combat attack to be a ``reckless attack''. This has \textlf{edge+} to \textlf{aim} but the character incurs \textlf{edge-} on \textlf{deflect} for one round.


\subsection{Shatter strike [2]}
(Requires \textlf{Weapon proficiency: unarmed}) The character gathers all their strength and makes a single unarmed attack, with only a single \textlf{damage check}, at a cost of 2 action points. If this attack causes damage then its victim is \textlf{vulnerable} to the next \textlf{damage check} it suffers.
...
\subsubsection{Upgrade: Steel tornado [2]}
This adds an additional \textlf{cleave} + 1 when using \textlf{Whirlwind of steel}.
...
Sometimes walking is just too boring. The character can move out of, or through, a combat area containing enemies without suffering a \textlf{moment of weakness}. To do so they must win an \textlf{opposed roll} with their \textlf{athletics} vs the highest \textlf{aim} enemy within the area in question. Failure results in a \textlf{moment of weakness}.  


\subsection{Adrenaline rush [3]}
A surge of adrenaline can grant a fighter great power in times of danger. If the character loses \textlf{endurance} to a \textlf{damage check}, they can spend a reaction point to gain \textlf{edge+} to their next \textlf{damage check}.
...
Is a good defence. When the character scores \textlf{critical success} on \textlf{deflect}, they may spend a reaction point so that the attacker suffers a \textlf{moment of weakness}. 
...
Now the bulls run from you. The character has learned to enter combat with unstoppable momentum. If the character moves in the same turn, or made a \textlf{run} action last round, they have \textlf{edge+} on \textlf{shove} actions (see Section~\ref{sec:adv-spc}) this round. 
...
Close-combat attacks against enemies who are \textlf{knocked down} or \textlf{grappled} gain \textlf{penetration} + 2 at a cost of 1 reaction point.
...
\subsection{Cunning plan [3]}
Better than other plans. When making \textlf{sneak attacks}, the character gains a \textlf{power} bonus of 1 plus their \textlf{cunning}.


\subsection{Decisive leadership [3]}
The character can spend a reaction point to use a \textlf{leadership} action at any time during combat.
...
It's hard to pin you down. This grants the character + 1 \textlf{deflect} provided they wear \textlf{light armour} or are unarmoured.
...
The character is skilled at moving and fighting in the chosen environment (X). This grants them \textlf{edge+} to stealth, as well as \textlf{survival} checks while in this setting. The character is also not affected by \textlf{rough} or \textlf{dangerous terrain} associated with this environment.
...
Don't tell me what to do. This allows the character \textlf{edge+} on checks against effects that would inflict \textlf{staggered}, \textlf{stunned}, \textlf{immobilised}, or \textlf{knocked-down}.
...
(Requires \textlf{Skill proficiency: leadership}) Whenever the character succeeds on a \textlf{leadership} check in combat they can restore 1 \textlf{endurance} to an ally within range 0.
...
If the character succeeds on a \textlf{feint} action, then associated \textlf{damage checks} gain any benefits that apply to \textlf{sneak attacks}.


\subsection{Merciless [4]}
The character shows no mercy, dispatching the weak with even greater abandon. \textlf{damage checks} against \textlf{bleeding}, \textlf{staggered}, or \textlf{stunned} targets gain a \textlf{lethality} bonus.


\subsection{Moving target [3]}
When mounted/driving and moving, the character has \textlf{edge+} to \textlf{deflect}.


\subsection{Nimble [3]}
\textlfirst{Nimble} characters are quick and agile. Their movement actions have +1 range while unarmoured or wearing \textlf{light armour}.


%\subsection{On Guard [3]}
%When the enemy strikes be prepared, when you strike be even more prepared. Every time the character \textlf{staggers} or \textlf{stuns} an enemy, they get \textlf{edge+} on their next \textlf{deflect} check. 
...
\begin{table}[ht!]
	...
	\begin{tabular}{|l|l|l|l|l|l|}
		...
		\hline
		Canine & 0 & 0 & 0 & 1 & Survival \\
		Big cat & 1 & 0 & 0 & 0 & Stealth \\
		Bird & 0 & 0 & 1 & 0 & Awareness  \\
		Reptile & 0 & 1 & 0 & 0 & Stealth \\ 
		\hline
	\end{tabular}
\end{table}
...
The character gains \textlf{edge+} on \textlf{Grapple} and \textlf{Shove} actions.


\subsection{Quick draw [2]}
The character can draw/change weapon at a cost of 1 reaction point.

\subsubsection{Upgrade: Shoot first [3]}
(Requires \textlf{Quick Draw}) The character's attacks, after making a surprise \textlf{Quick Draw} of \textlf{small} weapons are \textlf{sneak attacks}. This bonus cannot be claimed if the target is already fighting the character.


\subsection{Schadenfreude [4]}
The character gets \textlf{edge+} to \textlf{aim} versus enemies who have suffered damage this round.
...
When the character uses \textlf{defensive stance}, they have \textlf{edge+} on \textlf{deflect} for the next 2 attacks.


\subsection{Sixth sense [2]}
The character is so sharp they can evade even unseen attacks at the very last minute. This allows the character to attempt \textlf{deflect} against \textlf{sneak attacks}, however, they suffer \textlf{edge-} on this.


\subsection{Snake fist [4]}
(Requires \textlf{Weapon proficiency: unarmed}) The character has learned to unleash a volley of strikes with hands and feet. Unarmed attacks make an extra \textlf{damage check}. This does not incur \textlf{dual-wielding} penalties.
...
\subsection{Take-down [2]}
If you make a \textlf{shove} action against a target you have \textlf{grappled} they are slammed to the ground. This causes a successful \textlf{shove} to deal damage with normal \textlf{lethality} in addition to its usual effects. 
...
Pain is just information. When the character is reduced to 0 \textlf{endurance} they can spend a reaction point to regain 1 \textlf{endurance}. 


\subsection{Turn the tables [4]}
If the character is targeted by a \textlf{grapple} or \textlf{shove} attempt that fails they can spend a reaction point to successfully use this effect on the grappler/shover. 


\subsection{Two-weapon mastery [8]}
This allows the character to fight with two weapons simultaneously without \textlf{edge-} on \textlf{aim} checks. 


\subsection{Warrior's grace [5]}
The character may attack with \textlf{cumbersome} weapons at the cost of both an action and reaction point (rather than 2 action points). 


\subsection{Your mother was a hamster [3]}
The character has great knowledge of insults and is a master of taunting jibes. The character has \textlf{edge+} on \textlf{resolve} checks intended to taunt or provoke a target into attacking them.
