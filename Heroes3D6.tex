...
\item
A character must then choose 3 skills to be \textlf{proficient} in. These skills must be chosen from the ``associated skills'' list for the character's background. 
\item
...
These enemies are followers, they need to be lead. Without a leader to direct them, they are likely to be easily demoralised by powerful opponents or to simply attack the enemy they feel is the most threatening. Such behaviour is over-ridden in the presence of a leader. Mundane enemies suffer the \textlf{panic} effect (see section~\ref{sec:special}) if they run out of \textlf{endurance} or if they suffer a \textlf{badly-wounded} effect. Enemies who believe in their cause, or are mindless, are immune to this. When being lead, a mundane creature may use its leader's leadership score to make \textlf{panic} checks.
...
\subsubsection{Tactics}
Such enemies are independent, they do not need to be lead, but are more effective with a leader to direct them. These foes are unlikely to be easily demoralised or intimidated by powerful opposition and will coordinate and work together to defeat stronger opponents. suffer the \textlf{panic} effect (see section~\ref{sec:special}) if they run out of \textlf{endurance} or if they suffer a \textlf{badly-wounded} effect. Enemies who believe in their cause, or are \textlf{mindless}, are immune to this. When being lead, an elite creature may use its leader's leadership score to make \textlf{panic} checks.
...
A creature is declared \textlf{large} if it exceeds 3 m in the main dimension (height, length,width depending on creature geometry). Thus, such creatures can only hide in medium or better cover but gain \textlf{cleave} 2 on all close-combat attacks. A \textlf{large} creature has an innate bonus of +1 \textlf{power}. Such a creature has at least 4 \textlf{endurance} points.
...
These behemoths are all but inured to the attacks of normal weaponry. In consequence they have at least 8 \textlf{endurance} points. \textlf{Titanic} creatures have \textlf{cleave} 4 on all close-combat attacks and \textlf{massive damage} against smaller targets. \textlf{Titanic} creatures have an innate bonus of +3 \textlf{power} and the size threshold to be declared \textlf{titanic} is a requirement of least 12 m in the main dimension (height, length, or width depending on monster geometry). Such beasts have range 1 on their close-combat attacks. 
...
The character aims their strikes to maim the target. This is an attack action that costs 2 action points. Should the attack cause damage, the target is subject to the \textlf{cripple} effect. This effect applies only to creatures a maximum of 1 size category larger than the character.
...
The character may reduce the \textlf{reload} action point costs of ranged weapons by 1. If they do so, they experience an \textlf{edge} penalty on \textlf{aim} for attacks that benefit from this.
...
\subsubsection{Upgrade: Unrelenting [2]}
(Requires \textlf{Furious assault}) The character may use the \textlf{Furious assault} effect twice on a single attack action, the second use incurs an \textlf{edge} penalty to \textlf{deflect}. 


\subsection{Guard breaker [3]}
This attack knocks the foe off balance. The character may declare this special attack for 1 action point. If the attack causes damage then the target suffers an \textlf{edge} penalty to \textlf{deflect} for 1 round.
...
The character can declare a close-combat attack to be a ``reckless strike''. This has an \textlf{edge} bonus to \textlf{aim} but the character incurs and \textlf{edge} penalty on \textlf{deflect} for one round.
...
(Requires Bull-Headed) The character may make a damage roll against one victim of their successful \textlf{bull-rush} actions. 
...
(Requires \textlf{Duelist}) The character may gain an \textlf{edge} bonus on \textlf{aim} against any target whose attacks they \textlf{deflect} with \textlf{critical success}. You cannot claim this bonus if you use \textlf{Buckle your swash} on the same \textlf{deflected} attack.
...
It's hard to pin you down. This grants + 1 \textlf{deflect} provided they are not wearing \textlf{heavy armour}.
...
The character shows no mercy, dispatching the weak with even greater abandon. Damage rolls against \textlf{bleeding} or \textlf{crippled} targets gain a \textlf{lethality} bonus.
...
\subsection{Quick learner [5]}
Pick 3 weapon proficiencies for the character.
